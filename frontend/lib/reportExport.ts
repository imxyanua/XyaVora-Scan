...
export function generateMarkdownReport(report: ScanReport) {
  ...
  return [
    ...
    table([
      ...
      ["Languages", report.serverLocation.languages?.join(", ")],
      ["Currency", report.serverLocation.currencyCode ? `${report.serverLocation.currency} (${report.serverLocation.currencyCode})` : report.serverLocation.currency],
    ...
    table([
      ...
      ["Domain Expires", report.whois.expiryDate],
      ["Expiry Days Remaining", report.whois.expiryDaysRemaining],
      ["DNSSEC", report.whois.dnssec],
      ["Name Servers", report.whois.nameServers.join(", ")],
      ["WHOIS Evidence", report.whois.whoisEvidence?.join("; ")],
      ["robots.txt", report.siteDiscovery.robotsPresent],
      ["Sitemap", report.siteDiscovery.sitemapPresent],
      ["Discovery Evidence", report.siteDiscovery.discoveryEvidence?.join("; ")],
    ]),
    "",
    "## Cookies And Disclosure",
    "",
    table([
      ["Cookie Count", report.cookies.length],
      ["Cookie Evidence", report.cookies.flatMap((cookie) => cookie.evidence ?? []).join("; ")],
      ["security.txt Present", report.securityTxt.present],
      ["security.txt Location", report.securityTxt.location],
      ["security.txt Expires", report.securityTxt.expires],
      ["security.txt Expired", report.securityTxt.expired],
      ["security.txt Evidence", report.securityTxt.securityTxtEvidence?.join("; ")],
    ]),
    "",
    "## Tech Stack",
    "",
    ...
    table([
      ...
      ["Robots Directives", report.pageMetadata.robotsDirectives?.join(", ")],
      ["Canonical Host", report.pageMetadata.canonicalHost],
      ["Canonical Matches Final Host", report.pageMetadata.canonicalMatchesFinalHost],
      ["Noindex", report.pageMetadata.noindex],
      ["Nofollow", report.pageMetadata.nofollow],
      ["Metadata Evidence", report.pageMetadata.metadataEvidence?.join("; ")],
    ]),
    "",
    "## Security Headers",
    "",
